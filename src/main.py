...
class SearchInventorySchema(BaseModel):
    query: str = Field(default="", description="The search query (e.g., 'luxury SUV'). Can be empty if searching by car_name or price.")
    price_max: Optional[float] = Field(default=None, description="Maximum price of the car.")
    car_name: Optional[str] = Field(default=None, description="Specific name or model of the car (e.g., 'GV80').")

def search_inventory(query: str = "", price_max: float = None, car_name: str = None) -> str:
    results = retriever.retrieve(query, price_max=price_max, car_name=car_name)
    if not results:
...
tools = [
    {
        ...
        "function": {
            ...
            "description": "Search the Genesis inventory for cars matching the criteria. Use this to lookup cars before answering.",
            "parameters": SearchInventorySchema.model_json_schema()
        }
    }
]
...
def retrieve_node(state: State):
    ...
    speak("Hang on a minute, let me check our inventory for you...", interrupt_event=None)
    ...
    for tool_call in last_msg.tool_calls:
        if tool_call["name"] == "search_inventory":
            ...
            result_str = search_inventory(**args)
